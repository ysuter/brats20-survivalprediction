...
inpfeat.drop(columns=["Unnamed: 0"], inplace=True)
...
donedf = pd.read_csv("/media/yannick/c4a7e8d3-9ac5-463f-b6e6-92e216ae6ac0/BRATS/BraTS2020/concordanceidx_training3.csv")
donedf.set_index("Feature", inplace=True)
donefeatlist = donedf.index.values

todocols = list(set(featprocess_nosurv.columns.values) - set(donefeatlist))
print("Processing " + str(len(todocols)) + " features.")

cind_df = donedf
for col in tqdm(todocols):
    indexname = col
...
cind_df.to_csv('/media/yannick/c4a7e8d3-9ac5-463f-b6e6-92e216ae6ac0/BRATS/BraTS2020/concordanceidx_training_all.csv')
