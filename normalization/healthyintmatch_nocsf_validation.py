...
def normalizeimages(imgpath, seqprefix, intarray_target, squencemax):
    ...
    try:
        ...
        labelpath_eroded = os.path.join(os.path.split(imgpath)[0], currsubj + "_healthyseg_eroded.nii.gz")

        labelimg = sitk.ReadImage(labelpath)

        # erode label map
        gm_eroded = sitk.BinaryErode(labelimg, [1, 0, 0], sitk.sitkBall, 0, 2)
        label_eroded = sitk.BinaryErode(gm_eroded, [1, 0, 0], sitk.sitkBall, 0, 3)

        sitk.WriteImage(label_eroded, labelpath_eroded)

        labelstatfilt = sitk.LabelStatisticsImageFilter()
        # labelstatfilt.Execute(currimg, labelimg)
        labelstatfilt.Execute(currimg, label_eroded)
        ...
        outpath = os.path.join(os.path.split(imgpath)[0], seqprefix + '_pwlin-biascorr_gmwm_match_eroded.nii.gz')
...
# t1filename = '*t1.nii.gz'
# t1cfilename = '*t1ce.nii.gz'
# t2filename = '*t2.nii.gz'
...
# healthyseg = '*healthyseg.nii.gz'

# basedir = '/home/yannick/remoteubelix/MANAGE/data/postopprogrusable_normalized'
# basedir = '/media/yannick/c4a7e8d3-9ac5-463f-b6e6-92e216ae6ac0/MANAGE/data/postopprogrusable_normalized'
...
for img in tqdm(t1listkeep):
    ...
    labelpath_eroded =  os.path.join(os.path.split(img)[0], currsubj + "_healthyseg_eroded.nii.gz")
...
# save target intensities for processing of validation and test sets
savepath = '/media/yannick/c4a7e8d3-9ac5-463f-b6e6-92e216ae6ac0/MANAGE/code/MANAGE/brats2020/normalization'

print('# Normalizing T1c sequences')
intarray_target_t1c = [t1c_gm_overallmean, t1c_wm_overallmean]

with open(os.path.join(savepath, "T1c_norminfo_eroded.txt"), 'w') as filehandle:
    json.dump(str(intarray_target_t1c + [maxmean_t1c]), filehandle)

seqprefix = "CT1"
t1c_skipped = Parallel(n_jobs=num_cores)(
    delayed(normalizeimages)(t1cpath, seqprefix, intarray_target_t1c, maxmean_t1c) for t1cpath in tqdm(t1clistkeep))

print('### Skipped T1c:')
print(t1c_skipped)

print('# Normalizing T1 sequences')
intarray_target_t1 = [t1_gm_overallmean, t1_wm_overallmean]

with open(os.path.join(savepath, "T1_norminfo_eroded.txt"), 'w') as filehandle:
    json.dump(str(intarray_target_t1 + [maxmean_t1]), filehandle)
seqprefix = "T1"
t1_skipped = Parallel(n_jobs=num_cores)(
    delayed(normalizeimages)(t1path, seqprefix, intarray_target_t1, maxmean_t1) for t1path in tqdm(t1listkeep))

print('### Skipped T1:')
print(t1_skipped)

print('# Normalizing T2 sequences')
intarray_target_t2 = [t2_gm_overallmean, t2_wm_overallmean]
with open(os.path.join(savepath, "T2_norminfo_eroded.txt"), 'w') as filehandle:
    json.dump(str(intarray_target_t2 + [maxmean_t2]), filehandle)
seqprefix = "T2"
t2_skipped = Parallel(n_jobs=num_cores)(
    delayed(normalizeimages)(t2path, seqprefix, intarray_target_t2, maxmean_t2) for t2path in tqdm(t2listkeep))

print('### Skipped T2:')
print(t2_skipped)

print('# Normalizing FLAIR sequences')
intarray_target_flair = [flair_gm_overallmean, flair_wm_overallmean]
with open(os.path.join(savepath, "FLAIR_norminfo_eroded.txt"), 'w') as filehandle:
    json.dump(str(intarray_target_flair + [maxmean_flair]), filehandle)
seqprefix = "FLAIR"
flair_skipped = Parallel(n_jobs=num_cores)(
    delayed(normalizeimages)(flairpath, seqprefix, intarray_target_flair, maxmean_flair) for flairpath in tqdm(flairlistkeep))
...
validationsetpath = "/media/yannick/c4a7e8d3-9ac5-463f-b6e6-92e216ae6ac0/BRATS/BraTS2020/MICCAI_BraTS2020_ValidationData_onlysurvival"

flairlist_val = glob(os.path.join(validationsetpath, '*', flairfilename))
t2list_val = glob(os.path.join(validationsetpath, '*', t2filename))
t1list_val = glob(os.path.join(validationsetpath, '*', t1filename))
t1clist_val = glob(os.path.join(validationsetpath, '*', t1cfilename))

seqprefix = "CT1"
t1c_skipped_val = Parallel(n_jobs=num_cores)(
    delayed(normalizeimages)(t1cpath, seqprefix, intarray_target_t1c, maxmean_t1c) for t1cpath in tqdm(t1clist_val))
print('### Skipped T1c, validation set:')
print(t1c_skipped_val)

seqprefix = "T1"
t1_skipped_val = Parallel(n_jobs=num_cores)(
    delayed(normalizeimages)(t1path, seqprefix, intarray_target_t1, maxmean_t1) for t1path in tqdm(t1list_val))
print('### Skipped T1, validation set:')
print(t1_skipped_val)

seqprefix = "T2"
t2_skipped_val = Parallel(n_jobs=num_cores)(
    delayed(normalizeimages)(t2path, seqprefix, intarray_target_t2, maxmean_t2) for t2path in tqdm(t2list_val))
print('### Skipped T2, validation set:')
print(t2_skipped_val)

seqprefix = "FLAIR"
flair_skipped_val = Parallel(n_jobs=num_cores)(
    delayed(normalizeimages)(flairpath, seqprefix, intarray_target_flair, maxmean_flair) for flairpath in tqdm(flairlist_val))
print('### Skipped FLAIR, validation set:')
print(flair_skipped_val)
