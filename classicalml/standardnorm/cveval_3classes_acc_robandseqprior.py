...
balacc_df = pd.DataFrame(data=[], columns=["Feature selector", "ML method", "Split", "Parameter1", "Parameter2", "Accuracy"])
for split in np.arange(numsplits):
    ...
    for sel_name, sel in zip(selectornames_short, selectors):
        ...
        for clf_name, clf in zip(classifiernames, classifiers):
            ...
            print(balacc_df)
            balacc_df.to_csv(
                "/media/yannick/c4a7e8d3-9ac5-463f-b6e6-92e216ae6ac0/BRATS/BraTS2020/featsel_outputs/cvresults_robseqprior_acc.csv",
                index=False)

balacc_df.to_csv("/media/yannick/c4a7e8d3-9ac5-463f-b6e6-92e216ae6ac0/BRATS/BraTS2020/featsel_outputs/cvresults_robseqprior_acc.csv", index=False)
