        ...
        for clf_name, clf in zip(classifiernames, classifiers):
            ...
            if clf_name is "XGBoost":
                ...
                for c in tqdm(costparam):
                    clf = SVC(kernel="linear", C=c, verbose=0, max_iter=int(1e8))
                    clf.fit(X_train_selected, y_train)
                    # score = clf.score(X_test_selected, y_test)
                    if hasattr(clf, "decision_function"):
                        score = clf.decision_function(X_test_selected)
...
balacc_df.to_csv("/media/yannick/c4a7e8d3-9ac5-463f-b6e6-92e216ae6ac0/BRATS/BraTS2020/featsel_outputs/cvresults_acc.csv", index=False)
